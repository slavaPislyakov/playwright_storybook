...
## Тестовое покрытие (Storybook + Playwright)

Проект использует **Storybook Test Runner** на базе **Playwright** для автоматизированного тестирования компонентов. Ниже представлена детальная карта покрытия:

| Компонент | Сценарий (Story) | Что проверяется (Play function) |
| :--- | :--- | :--- |
| **Button** | Primary | Рендеринг, текст, отсутствие состояния загрузки, клик мышкой. |
| | Secondary | Проверка доступности (кнопка не заблокирована). |
| | Disabled | Блокировка кнопки: отсутствие реакции на клик. |
| | IconButton | Доступность для скринридеров (aria-label). |
| | KeyboardAccess | Навигация Tab (фокус), нажатие Enter, динамическая смена текста. |
| | AsyncSubmit | Асинхронная работа: Загрузка -> Отправлено. |
| | FullWidth | Геометрия: растягивание на всю ширину контейнера. |
| | IntentionalFail | (Тестовый) Проверка срабатывания ошибки в тестах. |
| **Input** | Default | Ввод текста и работа счетчика символов. |
| | RequiredValidation | Валидация: появление ошибки при потере фокуса. |
| | IntentionalFail | (Тестовый) Проверка ошибки при неверном счетчике. |
| **Modal** | Desktop | Наличие кнопок и их горизонтальное расположение. |
| | TabletIPad | Адаптивность: скрытие части кнопок на планшетах. |
| | MobileIPhone13Pro | Адаптивность: вертикальный стек кнопок на мобильных. |
